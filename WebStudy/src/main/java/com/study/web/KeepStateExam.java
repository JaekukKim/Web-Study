...
public class KeepStateExam extends HttpServlet {
	...
	public void service(ServletRequest request, ServletResponse response) throws ServletException, IOException {
		...
		if (op.equals("=")) {
			...
			/*
			 * 위의 x값을 저장한 부분이 핵심이라 생각한다. 지금 공부하는 부분이 ServletContext, 즉 application 저장소인데. 위의 코드는
			 * 어플리케이션 저장소에서 getAttribute를 이용해 저장된 "값" 즉 선대 서블릿이 setAttribute해놓고 떠나간 값을 가져오는과정의 코드인 것이다.
			 * 연산이 제대로 실행이 된다면 우리는 "값의 상태유지"에 성공한것이다.
			 */
			
			// y는 "지금 사용자가 전달한 value값"이 y이다.
			/*
				이 부분이 굉장히 ambiguous하다 (요즘 sql배워서 한번써봄 ㅎㅎ;) 지금 사용자가 전달한 값이라는건. getAttribute를 거치지 않고
				들어왔다는 얘기이다. 음. 처음에 들어온 값은 x에 저장되니 저장된 값을 "빼서"쓰는거고 y값은 저장 할 필요 없이 바로 쓰면 되니까
				String value_ = request.getParameter("value") => if (!value_.equals("")) { value = Integer.parseInt(value_); } 를 거치고 난 뒤의
				value를 바로쓰면 되는구나!!
			*/
			int y = value;

			String operator = (String) application.getAttribute("op");
			// getAttribute를 하기 위해선 반드시 setAttribute로 값을 "세팅" 해 주어야한다 만일 그렇지 않다면 null값을 반환한다. 
			int result = 0;
			...
			/*
			 * "value"라고 하는 키 값을 받아 String value_ = request.getParameter("value")
			 * -> value = Integer.parseInt(value_)의 과정을 거친 value를 저장한다.
			 */
